...
public class Optimize_Scheduler {

    private static List<Cloudlet> cloudletList;
    private static List<Vm> vmList;
    private static Datacenter[] datacenter;
    private static Optimize OptimizeSchedularInstance;
    ...
    private static List<Vm> createVM(int userId, int vms) {
        ...
        int mips = 1000; //cpu processing speed (million instructions per second)
    ...
    public static void main(String[] args) {
        Log.printLine("Starting Optimize Scheduler...");

        new GenerateMatrices();
        commMatrix = GenerateMatrices.getCommMatrix();
        execMatrix = GenerateMatrices.getExecMatrix();
        OptimizeSchedularInstance = new Optimize();
        mapping = OptimizeSchedularInstance.run();
        ...
        try {
            ...
            Optimize_DC_Broker broker = createBroker("Broker_0");
            ...
            Log.printLine(Optimize_Scheduler.class.getName() + " finished!");
        } catch (Exception e) {
            e.printStackTrace();
            Log.printLine("The simulation has been terminated due to an unexpected error");
        }
        
        SwingUtilities.invokeLater(new Runnable() {@Override public void run() {new FinalGraph1().setVisible(true);}});
         SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new FinalGraph().setVisible(true);
            }
        });
         SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new FinalGraph2().setVisible(true);
            }
        });
         ...
         SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new Final_Comparison1_Graph().setVisible(true);
            }
        });
         ...
         SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new Final_Comparison_Graph().setVisible(true);
            }
        });
    }

    private static Optimize_DC_Broker createBroker(String name) throws Exception {
        return new Optimize_DC_Broker(name);
    }
    ...
    private static void printCloudletList(List<Cloudlet> list) {
        ...
        OptimizeSchedularInstance.printBestFitness();
        //Added:
        Log.printLine("Total Completion Time: " + totalCompletionTime +" \n Avg Completion Time: "+ (totalCompletionTime/size));
        Log.printLine("Total Cost : " + totalCost+ " \n Avg cost: "+ (totalCost/size));
